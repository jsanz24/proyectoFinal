      ...
      <div>
          {/* {!this.state.startGame?<button onClick={(e) =>this.handleClick(e)}>start</button>:<div> */}
              {/* <p>Score: {JSON.stringify(this.state.score)}</p>  */}
                {this.state.speedX === 0?<div className="fondoDesktopWest"><div className="flexbox-container">{this.state.score.map((elem,idx) => <div >{idx+1} - {elem.id} - {elem.score}</div>)}</div></div>:
                    <div>
                        <div style={{position: "relative"}}>
                            <div className="objects">
                                <img alt="" className="gun shootAnimation" src="/img/west/gun.png" />
                            </div>
                        </div>
